...
import { IconBed, IconBath, IconRuler, IconCarGarage, IconSofa, IconMapPin2, IconDimensions, IconBuildingCommunity, IconBuildingWarehouse, IconElevator, IconAward } from '@tabler/icons-react';
...
const KeyFeatures: React.FC<KeyFeaturesProps> = ({ details }) => {
    ...
    features.push({
        icon: IconRuler,
        label: "Area",
        value: details.area ? `${details.area} ${getDisplayValue(areaUnitMap, details.area_unit)}` : "N/A",
        color: 'text-gray-600'
    });
    ...
    if (details.property_type === 'HOUSE' && propertyDetails) {
        ...
        if (houseDetails?.car_parking) {
            features.push({ icon: IconCarGarage, label: "Parking", value: houseDetails.car_parking.charAt(0).toUpperCase() + houseDetails.car_parking.slice(1).toLowerCase(), color: 'text-gray-600' });
        } else if (houseDetails?.num_carparking) {
            features.push({ icon: IconCarGarage, label: "Parking", value: `${houseDetails.num_carparking} space(s)`, color: 'text-gray-600' });
        }
        ...
        if (landDetails?.is_dtcp_approved !== undefined) {
            features.push({ icon: IconAward, label: "DTCP Approval", value: landDetails.is_dtcp_approved ? "Approved" : "Not Approved", color: 'text-gray-600' });
        }
