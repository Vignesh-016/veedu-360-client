...
const RentalApplicationModal: React.FC<RentalApplicationModalProps> = ({
    ...
    const validateForm = (): boolean => {
        ...
        try {
            const selectedDate = parseISO(moveInDate);
            if (selectedDate < addDays(today, 0)) {
                    ...
                    <div className="flex min-h-full items-center justify-center p-4 text-center">
                        ...
                        >
                            <DialogPanel className="w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                                ...
                                <p className="text-sm text-gray-500 mt-1 mb-4 truncate" title={propertyName}>
                                    For: {propertyName}
                                </p>
                                ...
                                <form onSubmit={handleSubmit} className="space-y-4 mt-4">
                                    <div>
                                        <label htmlFor="moveInDate" className="text-sm font-medium text-gray-700 mb-1 flex items-center">
                                            <IconCalendarEvent size={16} className="mr-1.5 text-gray-400" />Proposed Move-in Date <span className="text-red-500 ml-1">*</span>
                                        </label>
                                        <input
                                            type="date"
                                            id="moveInDate"
                                            ...
                                            className={getBaseInputClasses(!moveInDate && !!error)}
                                            required
                                            min={minMoveInDate}
                                            disabled={submitting}
                                        />
                                    ...
                                    <div>
                                        <label htmlFor="numOccupants" className="text-sm font-medium text-gray-700 mb-1 flex items-center">
                                            <IconUsers size={16} className="mr-1.5 text-gray-400" />Number of Occupants <span className="text-red-500 ml-1">*</span>
                                        </label>
                                        <input
                                            type="number"
                                            id="numOccupants"
                                            value={numOccupants}
                                            onChange={(e) => setNumOccupants(e.target.value === '' ? '' : parseInt(e.target.value))}
                                            className={getBaseInputClasses((numOccupants === '' || (typeof numOccupants === 'number' && numOccupants <=0)) && !!error)}
                                            required
                                            min="1"
                                            step="1"
                                            placeholder="e.g., 2"
                                    ...
                                    <div>
                                        <label htmlFor="applicantNotes" className="text-sm font-medium text-gray-700 mb-1 flex items-center">
                                            <IconClipboardText size={16} className="mr-1.5 text-gray-400" />Notes for Landlord/Agent (Optional)
                                        </label>
                                        <textarea
                                            id="applicantNotes"
                                            value={applicantNotes}
                                            onChange={(e) => setApplicantNotes(e.target.value)}
                                            className={`${getBaseInputClasses()} min-h-[80px]`}
                                            rows={3}
                                            maxLength={300}
                                            placeholder="Any specific requests or a brief introduction..."
                                            disabled={submitting}
                                        />
                                    ...
                                    <div className="text-xs text-gray-500 p-3 bg-gray-50 rounded-md border">
                                        ...
                                        Submitting this application indicates your strong interest. Our team will contact you for the next steps, which may include document verification.
                                    </div>

                                    <div className="mt-6 flex justify-end gap-3 pt-4 border-t border-gray-100">
                                        <button type="button" className={getSecondaryButtonClasses()} onClick={onClose} disabled={submitting}>
                                            Cancel
                                        </button>
                                        <button
                                            type="submit"
                                            className={getPrimaryButtonClasses()}
                                            disabled={submitting || !moveInDate || numOccupants === '' || (typeof numOccupants === 'number' && numOccupants <= 0)}
                                        >
