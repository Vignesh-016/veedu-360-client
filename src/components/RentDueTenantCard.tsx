import { MyRentDues, RentStatus } from '../lib/types';
...
function RentDueTenantCard({ rentDue }: RentDueTenantCardProps) {
    ...
    const formattedDueDate = format(parseISO(due_date), 'PPP'); // e.g., Jan 1, 2024
    ...
    const getStatusInfo = (status: RentStatus): { text: string; color: string; Icon: React.ElementType } => {
        switch (status) {
            ...
            case 'CANCELLED': return { text: 'Cancelled', color: 'gray', Icon: IconCircleCheck }; // Or a specific cancelled icon
    ...
    return (
        <div className={`border-l-4 border-${statusInfo.color}-500 bg-${statusInfo.color}-50 p-3 rounded-r-md shadow-sm my-2`}>
            ...
            {(landlord_name || landlord_email || landlord_phone) && (
                <div className="mb-2 pb-2 border-b border-gray-200/60 text-xs text-gray-600">
                    <span className="font-medium text-gray-500 block mb-0.5">Landlord Contact:</span>
                    {landlord_name && <span>{landlord_name}</span>}
                    {landlord_email && <span className='ml-1 flex items-center gap-1'><IconMail size={12} /> <a href={`mailto:${landlord_email}`} className="text-gray-600 hover:underline">{landlord_email}</a></span>}
                    {landlord_phone && <span className='ml-1 flex items-center gap-1'><IconPhone size={12} /> <a href={`tel:${landlord_phone}`} className="text-gray-600 hover:underline">+{landlord_phone}</a></span>}
                </div>
            )}
            ...
            {/* Amount Details */}
            <div className="flex flex-col sm:flex-row justify-between items-baseline text-sm mt-2 pt-2 border-t border-gray-200/60">
                ...
                {amount_paid <= 0 && status !== 'PAID' && status !== 'CANCELLED' && (
                    <div className="text-xs text-gray-600 mt-1 sm:mt-0">
                        (Not Paid)
                    </div>
                )}
...
export default RentDueTenantCard;