...
Deno.serve(async (req) => {
    ...
    try {
        console.log("Processing POST request for property document upload...");
        ...
        if (!userIsAuthorizedAdmin) {
            console.warn(`User ${adminUserId} is not an authorized admin for property document upload. Roles: ${adminRecord?.roles}`);
        ...
        console.log(`Admin ${adminUserId} authorized for property document upload.`);
        ...
        if (!propertyId || !documentFile || !documentType) {
            return new Response(JSON.stringify({ error: 'Missing property_id, document_file, or document_type' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }
        ...
        const filePath = `property_documents/${propertyId}/${uniqueFileName}`;
        ...
        // --- START MODIFICATION: Replace RPC with direct INSERT ---
        ...
        if (dbInsertError) {
            console.error(`Database insert error for property_documents:`, dbInsertError);
            try {
        ...
        if (!dbRecord || !dbRecord.document_id) {
            console.error(`Database insert for property_documents did not return document_id or record.`);
            try {
                await supabaseAdmin.storage.from(STORAGE_BUCKET_NAME).remove([filePath]);
        ...
        console.log(`Property document ${documentId} recorded for property ${propertyId}`);
        ...
        console.error('Overall Upload Property Document Error:', error, error.stack);
