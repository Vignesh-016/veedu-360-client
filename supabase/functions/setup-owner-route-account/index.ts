...
const canonicalIndianState=(raw:string)=>{const value=raw.trim().toLowerCase().replace(/[._-]+/g,' ').replace(/\s+/g,' '); const states:Record<string,string>={'tamil nadu':'Tamil Nadu','tamilnadu':'Tamil Nadu','tn':'Tamil Nadu','kerala':'Kerala','karnataka':'Karnataka','andhra pradesh':'Andhra Pradesh','telangana':'Telangana','maharashtra':'Maharashtra','delhi':'Delhi'}; return states[value]||raw.trim();};
Deno.serve(async(req)=>{
  ...
  const call=async(url:string,method='GET',payload?:unknown,step='razorpay_request')=>{const r=await fetch(`https://api.razorpay.com${url}`,{method,headers,body:payload?JSON.stringify(payload):undefined}); const d=await r.json().catch(()=>({})); if(!r.ok){const e=d?.error||{}; const err=new Error(`Razorpay request failed (${r.status}): ${e.description||e.reason||e.code||'Request rejected'}`); (err as any).step=step; (err as any).upstreamStatus=r.status; (err as any).safeCode=e.code; throw err;} return d;};
  const redactConfiguration=(configuration:any)=>{if(!configuration||typeof configuration!=='object')return configuration; const copy=JSON.parse(JSON.stringify(configuration)); const redact=(value:any)=>{if(!value||typeof value!=='object')return; for(const key of Object.keys(value)){if(['account_number','pan','api_key','api_secret'].includes(key.toLowerCase())) value[key]='[REDACTED]'; else redact(value[key]);}}; redact(copy); return copy;};
  const safeProductLog=(product:any)=>({activation_status:product?.activation_status||null,requirements:Array.isArray(product?.requirements)?product.requirements.map((r:any)=>({field_reference:r?.field_reference||null,reason_code:r?.reason_code||null,status:r?.status||null})):[],requested_configuration:redactConfiguration(product?.requested_configuration),active_configuration:redactConfiguration(product?.active_configuration)});
  try {
    ...
    const productId=payout.razorpay_product_id;
    if(!accountId||!productId) return out({error:'Existing Razorpay account and product IDs are required; no new IDs will be created.'},409);
    let product=await call(`/v2/accounts/${accountId}/products/${productId}`);
    const before=String(product.activation_status||product.status||'requested').toLowerCase();
    console.log('Razorpay product inspection:',JSON.stringify(safeProductLog(product)));
    const requirements=Array.isArray(product.requirements)?product.requirements:[];
    const settlementNeedsUpdate=requirements.some((r:any)=>String(r?.field_reference||'').toLowerCase().includes('settlement')&&['missing','invalid','required','needs_clarification'].includes(String(r?.status||r?.reason_code||'').toLowerCase()));
    const currentSettlement=product.active_configuration?.settlements||product.requested_configuration?.settlements;
    const bankChanged=JSON.stringify(currentSettlement||{})!==JSON.stringify({account_number:payout.account_number,ifsc_code:payout.ifsc_code,beneficiary_name:payout.account_holder_name});
    const bankConfigUpdated=settlementNeedsUpdate&&bankChanged;
    if(bankConfigUpdated) product=await call(`/v2/accounts/${accountId}/products/${productId}`,'PATCH',{settlements:{account_number:payout.account_number,ifsc_code:payout.ifsc_code,beneficiary_name:payout.account_holder_name},tnc_accepted:true},'bank_configuration');
    if(bankConfigUpdated) product=await call(`/v2/accounts/${accountId}/products/${productId}`);
    const status=String(product.activation_status||product.status||'requested').toLowerCase(); const eligible=status==='activated';
    ...
    const reason=eligible?null:status==='needs_clarification'?'Bank account verification requires attention.':status==='under_review'?'Verification Under Review':status==='verification_failed'?'Razorpay could not verify the submitted bank details.':'Razorpay is verifying the payout details.';
    await (supabaseAdmin as any).from('owner_payout_accounts').update({razorpay_product_status:status,payment_eligible:eligible,route_onboarding_error:reason,last_status_checked_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq('owner_user_id',target);
    console.log('Razorpay product inspection:',JSON.stringify(safeProductLog(product)));
    const safeProduct=safeProductLog(product);
    return out({account_id:accountId,product_id:productId,status_before:before,requirements:safeProduct.requirements,requested_configuration:safeProduct.requested_configuration,active_configuration:safeProduct.active_configuration,bank_config_updated:bankConfigUpdated,status_after:status,payment_eligible:eligible,action_required:reason});
