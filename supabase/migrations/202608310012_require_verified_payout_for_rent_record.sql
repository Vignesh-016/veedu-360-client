...
DECLARE v_id UUID; v RECORD; total BIGINT; admin_share BIGINT; owner_share BIGINT;
BEGIN
 IF NOT (public.current_user_has_role('accounts-team') OR public.current_user_has_role('super-admin')) THEN RAISE EXCEPTION 'Unauthorized: Insufficient privileges to create rent records.'; END IF;
 ...
 IF v.listing_type<>'RENTAL' THEN RAISE EXCEPTION 'Property is not a rental property.'; END IF;
 IF v.tenant IS NULL THEN RAISE EXCEPTION 'Property is not currently occupied by a tenant.'; END IF;
 IF v.submitter IS NULL THEN RAISE EXCEPTION 'Property does not have a valid owner.'; END IF;
