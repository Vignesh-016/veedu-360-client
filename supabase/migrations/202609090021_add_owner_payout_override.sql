ALTER TABLE public.rent_records
  ADD COLUMN IF NOT EXISTS split_override_applied BOOLEAN NOT NULL DEFAULT FALSE,
  ADD COLUMN IF NOT EXISTS owner_share_override_paise BIGINT,
  ADD COLUMN IF NOT EXISTS split_override_reason TEXT,
  ADD COLUMN IF NOT EXISTS split_override_by UUID,
  ADD COLUMN IF NOT EXISTS split_override_at TIMESTAMPTZ;

DROP FUNCTION IF EXISTS public.create_rent_record_admin(UUID,DATE,DATE,DATE,DECIMAL,TEXT);
CREATE FUNCTION public.create_rent_record_admin(p_property_id UUID,p_due_date DATE,p_period_start_date DATE,p_period_end_date DATE,p_amount_due DECIMAL,p_notes TEXT DEFAULT NULL)
RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
BEGIN RETURN public.create_rent_record_admin(p_property_id,p_due_date,p_period_start_date,p_period_end_date,p_amount_due,p_notes,NULL,NULL); END; $$;
CREATE OR REPLACE FUNCTION public.create_rent_record_admin(p_property_id UUID,p_due_date DATE,p_period_start_date DATE,p_period_end_date DATE,p_amount_due DECIMAL,p_notes TEXT,p_owner_payout_override DECIMAL DEFAULT NULL,p_override_reason TEXT DEFAULT NULL)
RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
...
BEGIN
 ...
 IF p_owner_payout_override IS NOT NULL THEN
   ...
   override_paise:=ROUND(p_owner_payout_override*100)::BIGINT; IF override_paise<0 OR override_paise>total THEN RAISE EXCEPTION 'Owner payout override must be between zero and the rent amount.'; END IF;
