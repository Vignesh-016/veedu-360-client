-- Razorpay webhooks are authenticated with RAZORPAY_WEBHOOK_SECRET, not with
-- the checkout signature sent to the browser. The webhook must therefore be
-- able to complete a property-management payment without that optional value.
-- Recreate the function with the same return contract and an optional third
-- argument. An explicit search_path prevents object-shadowing in this
...
REVOKE ALL ON FUNCTION public.complete_property_management_payment(TEXT, TEXT, TEXT) FROM PUBLIC;
