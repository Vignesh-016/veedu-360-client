-- Expose the persisted payout requirement to both applications.
...
RETURNS TABLE (plan_id UUID, name TEXT, percentage NUMERIC, description TEXT, is_active BOOLEAN,
  ...
  document_processing_fee_enabled BOOLEAN, display_order INTEGER, requires_payout_account BOOLEAN)
...
BEGIN
  ...
  RETURN QUERY SELECT m.plan_id,m.name,m.percentage,m.description,m.is_active,m.created_at,m.updated_at,
    ...
    COALESCE(m.requires_payout_account,FALSE)
